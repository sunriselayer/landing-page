<script lang="ts">
	...
	const matrics = Array.from({ length: 3 }).map((_, i) => {
		return {
			index: i,
			title: `key-matrics.matric-${i + 1}.title`,
			label: `key-matrics.matric-${i + 1}.label`,
			description: `key-matrics.matric-${i + 1}.description`,
			imgSrc: (images as Record<string, string>)[`matric${i + 1}`]
		};
	});

	const ultraSteps = Array.from({ length: 4 }).map((_, i) => {
		return {
			index: i,
...
<main class="md:h-[982px] justify-center">
	...
	>
		...
		>
			<h3
				class="flex gap-3 items-center normal-text font-bold md:text-[1.5625rem] text-lg leading-none"
			>
	...
	>
		<h2
			class="normal-text md:text-center font-bold md:text-[3.125rem] text-[2rem] md:leading-none leading-[1.3125rem] md:pt-[7.5rem] md:pb-20 pb-6"
		>
			{@html $_('key-matrics.title')}
		</h2>
		<div class="flex flex-row md:gap-4 gap-2 md:pb-[3.75rem] pb-6">
			{#each matrics as matric}
				<div
				...
				>
					<div>
						<h3
							class="highlight-text font-bold font-eudoxus md:text-[3.375rem] text-[1.5rem] leading-none"
						>
							{@html $_(matric.title)}
						</h3>

						<h6 class="normal-text md:font-medium md:text-[2rem] text-[0.875rem] leading-none">
							{@html $_(matric.label)}
						</h6>
					</div>

					<img class="w-[60%] right-0 bottom-0 absolute" src={matric.imgSrc} alt="" />
					<p class="normal-text items-start md:text-[1.625rem] md:leading-[1.58rem] text-[0.75rem] leading-[0.7rem]">
						{@html $_(matric.description)}
					</p>
				</div>
			{/each}
		</div>
	...
	<section class="z-10">
		<div
			class="md:w-[100%] md:mt-[-16.625rem] md:pt-[16.625rem] mt-[-10rem] pt-[7rem] bg-[#FFF1CB] md:pb-[7.5rem]"
		>
			<div class="mx-auto lg:w-[920px] w-[calc(100vw-2.5rem)]">
				<div class="md:pt-[5.875rem] md:pb-20 md:px-[2.3125rem] pb-10 pt-6">
					...
					<p class="normal-text text-center md:text-[1.5rem] md:leading-7 text-[0.875rem] leading-none">
						{@html $_('fundamentals.description')}
					</p>
				</div>
				<div class="grid grid-cols-2 md:gap-4 gap-2">
					{#each ultraSteps as step}
						<div
							class="md:min-h-[9.625rem] flex-1 bg-[#FFF9EA] rounded-lg overflow-clip relative justify-between p-6"
						>
							<img class="md:w-7 w-[0.875rem]" src={step.icon} alt="" />
							<h3
								class="md:pt-5 md:pb-4 highlight-text font-bold font-eudoxus md:text-[2rem] text-[1rem] leading-none"
							>
								{@html $_(step.label)}
							</h3>
