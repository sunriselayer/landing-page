	...
	<section class="w-full relative">
		...
		<div class="relative">
			{#each topSteps as step}
				<div
					id={step.id}
					class="roadmap-steps bottom-3 px-[0.2675rem]"
				>
					<p class="highlight-text font-bold tracking-wider md:text-[1.25rem] text-[9.45px] md:leading-5">
						{$_(step.title)}
					</p>
					<p class="md:text-xs text-[6.14px] normal-text text-opacity-70">{@html $_(step.description)}</p>
				</div>
			{/each}
		</div>
		<div class="flex border border-[#FFC851] bg-[#FFF4D5] justify-between items-center md:px-5 px-[6px] py-[5.24px] md:py-4">
			<p class="highlight-text md:text-[1.5rem] text-[7.28px] font-bold tracking-wider">{start}</p>
			{#each months as m}
				<span id={`${m}_${start}`} class="md:text-sm normal-text text-[4.66px] uppercase font-medium">{$_(m)}</span>
			{/each}
			<p class="highlight-text md:text-[1.5rem] text-[7.28px] font-bold tracking-wider">{end}</p>
			{#each months.slice(0, 6) as m}
				<span id={`${m}_${end}`} class="md:text-sm normal-text text-[4.66px] uppercase font-medium">{$_(m)}</span>
			{/each}
		</div>
		<div class="relative">
			{#each bottomSteps as step}
				<div
					id={step.id}
					class="roadmap-steps mt-3 px-[0.3125rem]"
				>
					{#if typeof step.description !== 'string'}
                        {#each step.description as desc}
                            <p class="md:text-xs text-[6.14px] normal-text text-opacity-70 md:pr-12 pr-4">{@html $_(desc)}</p>
                        {/each}
                    {:else}
						<p class="md:text-xs text-[6.14px] normal-text text-opacity-70 md:pr-12 pr-4">{@html $_(step.description)}</p>
					{/if}
					<p class="highlight-text font-bold tracking-wider md:text-[1.25rem] text-[9.45px] md:leading-5">
						{$_(step.title)}
					</p>
